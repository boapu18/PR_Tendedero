...
function DetailReport() {
    ...
    const handleDeleteReport = async () => {
        const confirmation = await confirmationAlert(
            "¿Está seguro que desea eliminar la denuncia?"
        );
        ...
        try {
            if (confirmation) {
                ...
                successAlert(response.data.message, false, () => {
                    handleBack();
                });
            }
        } catch (error) {
            ...
            {!loading && !error && reportData && (
                <>
                    ...
                    <p className="text-muted mb-4">
                        Fecha de registro:{" "}
                        {new Date(reportData.creationDate).toLocaleString("es-CR")}
                    </p>
